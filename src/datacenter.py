...
class DataCenter:
    ...
    def _initialize_state_points(self):
        ...
        return {
            ...
            "T10_cw_to_chiller": self.config["t_wb_ambient"]
            + self.config["cooling_tower_approach"],
        }
    ...
    def _calculate_design_flow_rates(self):
        ...
        return {"m_gpu": m_gpu, "m_chw": m_chw, "m_cw": m_cw, "m_air": m_air}
    ...
    def solve_steady_state(self, utilization=1.0, t_wb=None, max_iter=100, tolerance=0.01):
        ...
        for iteration in range(max_iter):
            ...
            tower_result = self.cooling_tower.solve(
                q_cond=q_cond, m_dot_cw=self.flow_rates["m_cw"], t_in=t_cw_from_chiller, t_wb=t_wb
            )
    ...
    def print_summary(self, results):
        ...
        print("\n--- WATER CONSUMPTION ---")
        print(
            f"Evaporation:          {results['m_evap_kg_s']:>8.1f} kg/s  ({results['m_evap_kg_s']*3600:>10,.0f} L/hr)"
        )
        print(
            f"Blowdown:             {results['m_blowdown_kg_s']:>8.1f} kg/s  ({results['m_blowdown_kg_s']*3600:>10,.0f} L/hr)"
        )
        print(
            f"Total Makeup:         {results['m_makeup_kg_s']:>8.1f} kg/s  ({results['m_makeup_L_hr']:>10,.0f} L/hr)"
        )
        print(f"WUE:                  {wue:>8.3f} L/kWh")
        print(
            f"Annual Water:         {results['m_makeup_kg_s']*3600*8760/1e9:>8.1f} million m³/year"
        )
        ...
        print(
            f"GPU Coolant Out:      {results['T_gpu_out_C']:>8.1f}°C  (limit: {self.gpu_load.max_temp:.0f}°C) {'✓' if results['gpu_temp_ok'] else '✗'}"
        )
        print(
            f"Building Air Out:     {results['T_air_out_C']:>8.1f}°C  (limit: {self.building_load.max_temp:.0f}°C) {'✓' if results['building_temp_ok'] else '✗'}"
        )
        print(
            f"CHW Supply/Return:    {results['state_points']['T1_chw_supply']:>8.1f}°C / {results['state_points']['T4_chw_return']:.1f}°C"
        )
        print(
            f"CW Range:             {results['state_points']['T9_cw_from_chiller'] - results['state_points']['T8_cw_from_tower']:>8.1f}°C"
        )
        print(
            f"CT Approach:          {results['state_points']['T8_cw_from_tower'] - results['T_wb_C']:>8.1f}°C"
        )

        print("\n--- VALIDATION ---")
        print(f"Energy Balance Error: {results['energy_balance_error_pct']:>8.4f}%")
        print(
            f"Convergence:          {'Yes' if results['converged'] else 'No'} ({results['iterations']} iterations)"
        )
        if not results["converged"]:
            print(
                f"Max Temperature Change: {results['max_change_C']:>6.4f}°C (tolerance: {0.01}°C)"
            )
